...
Cada Avaliação da IA registra qual versão de prompt a produziu. A escrita direta usa uma operação transacional do PostgreSQL para validar a Régua, recalcular a nota e persistir Avaliação e checks sem estados parciais. A chave do OpenRouter, a Credential ElevenLabs do n8n (ingestão/reconciliação) e a credencial usada pelo n8n para essa escrita ficam no cofre de Credentials do n8n; a `ELEVENLABS_API_KEY` do `.env` do HQ é exclusiva do Monitoramento ao Vivo (ADR-0005) e não substitui a Credential do n8n. O HQ usa sua própria credencial para suas responsabilidades. Indisponibilidade do banco interrompe a avaliação automática; o HQ continua sem responsabilidade de execução ou retentativa do LLM.
