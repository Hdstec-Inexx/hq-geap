...
test('filtros da Fila de Curadoria fixam dia civil America/Sao_Paulo e motivo', async () => {
  ...
  assert.deepEqual(comConversationId.values, [
    '2026-09-01',
    '2026-09-30',
    'conv-fila-xyz'
  ]);
...
test('filtros da Fila sem inicio usam o mes civil corrente em America/Sao_Paulo', async () => {
  const { buildFilaCuradoriaFilters } = await import(
    '../../apps/api/src/modules/curadoria/repository.js'
  );

  const now = new Date('2026-09-10T18:00:00.000Z');
  const mesCorrente = buildFilaCuradoriaFilters(
    { conversationId: 'conv-fila-xyz' },
    1,
    { now }
  );
  assert.match(
    mesCorrente.clauses.join(' and '),
...
test('buildCuradoriasRealizadasFilters sem datas nao aplica mes corrente implicito', async () => {
  ...
  assert.doesNotMatch(filters.clauses.join(' and '), /concluido_em/);
